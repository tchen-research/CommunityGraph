...
def main():
    ...
    # ---- people ------------------------------------------------------------
    # Entries with `hidden: true` stay in the data files (so they can be
    # restored by deleting the flag) but are excluded from the build, along
    # with their papers, advising records, and connections.
    by_id = {}
    hidden_ids = set()
    for p in people:
        pid = p.get("id")
    ...
    # ---- papers ------------------------------------------------------------
    hidden_stats = {"papers": 0, "advising": 0, "connections": 0}
    ...
    for raw in papers_in:
        ...
        n_before_hiding = len(authors)
        authors = [a for a in authors if a not in hidden_ids]
        if len(authors) < 2:
            if n_before_hiding >= 2:
                hidden_stats["papers"] += 1  # only hidden people kept it alive
            else:
    ...
    for rec in advising_in:
        ...
        if kind not in ADVISING_VALUE:
            ...
            continue
        if a in hidden_ids or s in hidden_ids:
            hidden_stats["advising"] += 1
            continue
    ...
    for c in connections:
        ...
        if missing:
            ...
            continue
        if a in hidden_ids or b in hidden_ids:
            hidden_stats["connections"] += 1
            continue
        key = pair_key(a, b)
        if key in edges:
    ...
    isolated = sorted(set(by_id) - connected - hidden_ids)
    if isolated:
    ...
    if hidden_ids:
        print(f"  hidden: {len(hidden_ids)} people "
              f"(excluded {hidden_stats['papers']} papers, "
              f"{hidden_stats['advising']} advising, "
              f"{hidden_stats['connections']} connections)")
    print(
