...
(() => {
  // data.js is loaded from here rather than a static <script> tag so the
  // loader gets painted and composited BEFORE the main thread blocks on
  // parsing the data and building the graph - the spinner keeps animating
  // on the compositor throughout.
  function boot() {
    const s = document.createElement("script");
    s.src = "data.js";
    s.onload = init;
    s.onerror = init; // init's missing-DATA guard shows the error state
  ...
  if (!DATA) {
    ...
    document.getElementById("layout").classList.remove("booting");
  ...
  function refreshForces(reheat = true) {
    ...
    sim.force("link")
      .links(visible)
      .distance((l) => 66 + 209 * (1 - l.rel))
      .strength((l) => 0.06 + 0.4 * l.rel);
    sim.force("collide").radius((d) => d.r + 10);
  ...
  DATA.factors.forEach((f) => {
    ...
    input.type = "range";
    input.min = 0; input.max = f.slider_max || 1; input.step = 0.05;
    ...
    if (f.bonus) {
      ...
      panel.hidden = false;
      panel.querySelector("#bonus-sliders").appendChild(row);
    } else {
  ...
  function edgeHtml(l) {
    const rows = DATA.factors.filter((f) => !f.bonus).map((f) => {
      const raw = f.compute === "papers"
        ? (l.papersInRange || []).length
        : (l.factors[f.id] || 0);
      const norm = f.saturation
        ? raw / (raw + f.saturation)
        : Math.min(raw, f.max) / f.max;
      const contrib = weights[f.id] * norm * (f.scale || 1);
  ...
  // everything is built: drop the loader and reveal the whole UI at once
  document.getElementById("layout").classList.remove("booting");
  document.getElementById("loader").remove();
  }

  // double-rAF guarantees the loader has been painted (and its spinner
  // handed to the compositor) before any heavy work starts
